...
#******************************************************************************#
#*********** Different Windows Around Placebo Adoption Times (Day) ************#
#******************************************************************************#

dataALL = read.dta13("Data_daily_barrio.dta")
dim(dataALL)

dates = c("2015-11-04","2016-11-02","2017-11-01","2018-11-07")
...
results_AM1 = results[,-c(4,7,10)]
print(results_AM1)

colnames(results_AM1) <- c(
  "window",
  "delta_crime.15", "pval_crime.15",
  "delta_crime.16", "pval_crime.16",
  "delta_crime.17", "pval_crime.17",
  "delta_crime.18", "pval_crime.18"
)
...
for (year in 15:18) {
  ...
  plot_title <- if (year == 17) {
    "Intervention: 1st Wed. of Nov. 2017"
  } else {
    paste0("Placebo: 1st Wed. of Nov. 20", year)
  }
...
all <- grid.arrange(
  arrangeGrob(
    grobs = list(plot_15_1, plot_16_1, plot_17_1, plot_18_1),
    nrow = 1,
    top = textGrob("Treatment Reversal Assignment Mechanism", gp = gpar(fontsize = 14, fontface = "bold"))
  ),
  arrangeGrob(
    grobs = list(plot_15_2, plot_16_2, plot_17_2, plot_18_2),
    nrow = 1,
...
ggsave("tables-and-plots/CDT_Figure_SA_PlaceboDay.png", plot = all, width = 18, height = 9, dpi = 600)
